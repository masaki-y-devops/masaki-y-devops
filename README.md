### 【個人活動・システム運用実績】
Linux（Arch, NixOS, Ubuntu Server）を用いた自宅サーバーの構築およびマルチコンテナ環境の運用の経験があります。単にツールを動かすだけでなく、インフラの再現性や運用の自動化（SREマインド）を意識した設計・トラブルシューティングを強みとしています。

<details>
<summary> 詳しいスキルセットや経歴を見る（クリックで展開）</summary>
...
###【趣味・関心】自作PC・UMPCのカスタマイズ： ハードウェアの仕様（TDP、排熱、アーキテクチャ）に関心があり、小型筐体におけるリソースの最適化を楽しんでいます。OSのチューニング： UMPCや自宅サーバーにArch LinuxやNixOSを導入し、ハードウェアの性能を最大限に引き出すためのミニマルな環境構築やカーネル設定、自動化スクリプトの作成を趣味として行っています。
